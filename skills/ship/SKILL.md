---
name: ship
description: breakdown.md の PR を 1 PR ずつ executor で実装し、完了したマイルストーンをチェックするループ
argument-hint: ""
---

## 手順

`.belt/breakdown.md` の未チェック PR を先頭から 1 つずつ `executor` で実装し、各 PR を `reviewer` + `ai-antipattern-reviewer` の最小レビューに通してからチェックする。全 PR が完了したら roadmap.md の対象マイルストーンをチェック済みにし、breakdown.md を削除する。

`/belt:breakdown` で 1 マイルストーンを PR 粒度に分解したあと、その PR 群を消化するための専用スキル。マイルストーン単位で進める場合は `/belt:cruise` を使う。

breakdown 済みの PR は既に 1 PR で完結する小さな実装単位に分解されているため、ship は autopilot のような多段オーケストレーションを使わず実装を executor 1 段に絞る。品質は **PR が breakdown の受け入れ基準・スコープを満たしているか / 先行 PR との統合が壊れていないか** を確認する後段の 2 レビューゲートで担保する。
...
Read ツールで `.belt/roadmap.md` を読み込み、先頭の「アーキテクチャ方針」セクションを抽出して保持する（各 executor 実行時のコンテキストに使う）。roadmap.md が存在しない場合はアーキテクチャ方針なしで続行する。
...
オーケストレーターが Write ツールで `.belt/phases/prompts/ship-executor-i{iteration}.md` を作成する:

```markdown
## タスク

以下の PR を 1 PR で完結する単位として実装してください。実装はリポジトリに直接行うこと。
...
executor 完了後、Step 3（PR レビュー）へ進む。

---

### Step 3: PR レビュー (reviewer + ai-antipattern-reviewer)

executor が完了した PR を、マージ前提の単位として最小レビューに通す。Step 2 で +1 した `iteration` をそのまま使い、この Step 内で作成する差分・プロンプト・出力ファイルもすべて同じ `iteration` 番号にする。

1. **差分の取得**: Bash ツールで `git --no-pager diff {baseline}` と `git --no-pager diff --stat {baseline}` を実行し、PR 開始時点からの変更を取得する。出力を Write ツールで `.belt/phases/outputs/ship-pr{N}-changes-i{iteration}.md` に保存する。差分が空の場合は executor が実装を行っていない異常なので、ユーザーに報告して `status="error"`, `active=false` で終了する。
...
4. **ルーティング**（両出力の最終行のステータスタグで機械的に判定する）:
   ...
   - いずれかが `[STATUS:REQUEST_CHANGES]`: 修正のため Step 2 に戻って executor を再実行する。再実行では `iteration` が +1 され、`ship-executor-i{iteration}.md` プロンプトの末尾にレビュー指摘への参照（前 `iteration` の `ship-reviewer-i{iteration}.md` と `ship-antipattern-i{iteration}.md` を Read して反映するよう指示）を追加する。完了後この Step 3 を新しい `iteration` で再レビューする。リトライは最大 2 回。リトライ時、レビューアは前回出力を参照に追加して finding_id を `new / persists / resolved` で追跡する。2 回リトライしても `REQUEST_CHANGES` が残る場合、未解決の指摘をユーザーに提示し、`status="error"`, `active=false` で終了する。
