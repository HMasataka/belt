...
手順開始前に Read ツールで以下のリファレンスを読み込む:

- `skills/autopilot/references/policies.md` — 共通ポリシー（コーディング、テスト、レビュー、ファクトチェック、AI 生成コード注意事項）

読み込んだ内容はプロンプトファイル作成時に、ポリシーが必要なエージェント（executor, test-engineer, debugger, reviewer, security-reviewer, ai-antipattern-reviewer）のプロンプトファイルに以下のセクションを追加する:
...
analyst, architect, planner, critic にはポリシー参照を含めない（分析・設計フェーズではコーディングポリシーは不要）。ただし architect, planner にはファクトチェックポリシーのセクションのみをプロンプトファイルにインラインで埋め込む:

```markdown
## ファクトチェック

分析・設計で使用する情報は必ずソース・オブ・トゥルースで裏取りすること。コードの振る舞い→実際のソースコード、設定値→実際の設定ファイル、API→実際の実装コード、型→型定義ファイル、依存関係→パッケージマニフェスト。推測で判断しない。
```
...
設計で使用する情報は必ずソース・オブ・トゥルースで裏取りすること。コードの振る舞い→実際のソースコード、設定値→実際の設定ファイル、API→実際の実装コード、型→型定義ファイル、依存関係→パッケージマニフェスト。推測で判断しない。
...
計画で使用する情報は必ずソース・オブ・トゥルースで裏取りすること。コードの振る舞い→実際のソースコード、設定値→実際の設定ファイル、API→実際の実装コード、型→型定義ファイル、依存関係→パッケージマニフェスト。推測で判断しない。
...
オーケストレーターが Write ツールで `.belt/phases/prompts/reviewer.md`、`.belt/phases/prompts/security-reviewer.md`、`.belt/phases/prompts/ai-antipattern-reviewer.md` にプロンプトファイルを作成する:
...
AI 生成コード特有のアンチパターンをレビューすること。幻覚 API、スコープクリープ、デッドコード、フォールバック濫用、不要な後方互換対応を重点的に検出する。
...
作業結果を `.belt/phases/outputs/ai-antipattern-reviewer.md` に Write ツールで保存してください。
```
...
Task(
  subagent_type="belt:ai-antipattern-reviewer",
  prompt="`.belt/phases/prompts/ai-antipattern-reviewer.md` を Read ツールで読み、指示に従ってください。作業結果を `.belt/phases/outputs/ai-antipattern-reviewer.md` に Write ツールで保存してください。"
)
```
...
### レビュー
[コードレビューの判定 + セキュリティレビューの判定 + AI Antipattern レビューの判定、主要な発見事項]
```
