...
`model="opus"` を固定で指定して Task を起動する。dispatch の executor は autopilot のような上流の分析・設計フェーズを経ずいきなり実装に入るため、判断品質を確保する目的で complexity によらず opus を使う（autopilot 内の executor は上流フェーズがあるため sonnet 既定のまま）:
