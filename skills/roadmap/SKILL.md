...
機能要件・非機能要件・エッジケース・リスクの各セクションからチェック済み (`- [x]`) の項目のみを抽出し、未チェック (`- [ ]`) の項目は除外する。チェック済み項目が1つもない場合、ユーザーに「spec.md で要件にチェックを入れてから再実行してください」と案内して終了する。

Open Questions セクションはチェックボックス形式ではないため、項目をそのまますべて抽出して保持する（未解決事項として後続フェーズに伝える）。技術コンテキストのセクションもそのまま保持する。
