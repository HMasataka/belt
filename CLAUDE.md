...
ランタイムの作業ディレクトリは利用者プロジェクト側であり、プラグインは利用者プロジェクトに `.belt/` を作って状態を書き込む（`.belt/state.json`、`.belt/phases/{prompts,outputs}/`、`.belt/spec.md`、`.belt/roadmap.md`）。
...
4. ユーザーがロードマップを確認
5. `/belt:cruise` — 未チェックタスクを含む最初のマイルストーンを autopilot に渡し、完了したらチェックボックスを更新、次のマイルストーンへ。途中再開可能
...
| critic                             | opus   | 計画品質ゲート                                             |
| executor                           | sonnet | 実装。complexity: high のときだけ opus に切り替える        |
| test-engineer / debugger           | sonnet | テスト作成 / 失敗修正                                      |
| reviewer / ai-antipattern-reviewer | sonnet | コード品質 / AI 生成アンチパターン                         |
